...
import org.xbib.graphics.barcode.impl.DrawingUtil;
...
public class UPCEANCanvasLogicHandler implements ClassicBarcodeLogicHandler {
    ...
    private UPCEANGenerator bcBean;
     ...
     */
    public UPCEANCanvasLogicHandler(AbstractBarcodeGenerator bcBean, Canvas canvas) {
        if (!(bcBean instanceof UPCEANGenerator)) {
            throw new IllegalArgumentException("This LogicHandler can only be "
                + "used with UPC and EAN barcode implementations");
        }
        this.bcBean = (UPCEANGenerator)bcBean;
        this.canvas = canvas;
    }
    ...
    private boolean isEAN() {
        return (bcBean instanceof EAN13Generator) || (bcBean instanceof EAN8Generator);
    }
