...
public class WithArticlesPipeline implements Pipeline<SerialItemPipelineElement> {
    ...
    public WithArticlesPipeline(WithArticles service, int number) {
   ...
   /* @Override
    ...
    public String toString() {
        return WithArticlesPipeline.class.getSimpleName() + "." + threadId;
    }
    ...
    private void process(SerialItem serialItem) throws IOException {
        ...
        if (service.settings().get("xref-index") != null) {
    ...
    private void fetchMedline(QueryBuilder queryBuilder, Map<String,Map<String,Object>> docs) throws IOException {
        ...
        */
        SearchRequestBuilder searchRequest = service.client().prepareSearch()
                .setIndices(service.settings().get("medline-index"))
                .setTypes(service.settings().get("medline-type"))
    ...
    private void fetchXref(QueryBuilder queryBuilder, Map<String,Map<String,Object>> docs) throws IOException {
        ...
        */
        SearchRequestBuilder searchRequest = service.client().prepareSearch()
                .setIndices(service.settings().get("xref-index"))
                .setTypes(service.settings().get("xref-type"))
                .setQuery(queryBuilder)
                .setSize(service.size()) // size() is per shard
                .setSearchType(SearchType.SCAN)
                .setScroll(TimeValue.timeValueMillis(service.millis()));
    ...
    private void process(SerialItem serialItem, Map<String,Map<String,Object>> docs) throws IOException {
        ...
        for (Map.Entry<String, Map<String, Object>> entry : docs.entrySet()) {
            ...
            builder.value(doc);
            String index = service.settings().get("target-index");
            String type = service.settings().get("target-type");
            String id = entry.getKey();
            service.ingest().index(index, type, id, builder.string());
        }
    }
    ...
    private void setPagination(Map<String,Object> map, Integer begin, Integer end) {
        List<Map<String,Object>> newList = newLinkedList();
    ...
    private void mergeDOIs(Map<String,Object> map1, Map<String,Object> map2) {
        List<String> newDOIS = newLinkedList();
