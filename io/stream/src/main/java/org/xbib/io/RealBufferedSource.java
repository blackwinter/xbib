...
final class RealBufferedSource implements BufferedSource {
    ...
    public byte[] readToDelimiter(byte delimiter) throws EOFException {
        return buffer.readToDelimiter(delimiter);
    }
    ...
    public long readToDelimiter(Sink sink, byte delimiter, long fromIndex) throws IOException {
        if (closed) {
            throw new IllegalStateException("closed");
        }
        if (sink == null) {
        ...
        long totalBytesWritten = 0;
        while (buffer.indexOf(delimiter, fromIndex) == -1) {
            fromIndex = buffer.size();
            if (fromIndex > 0) {
                totalBytesWritten += fromIndex;
                sink.write(buffer, fromIndex);
            }
            if (source.read(buffer, Segment.SIZE) == -1) {
                return -1L;
            }
    ...
    public String readUtf8LineStrict() throws IOException {
        ...
        if (newline == -1L) {
            ...
            throw new EOFException("newline not found: size=" + buffer.size()
