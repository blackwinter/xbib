...
import static org.xbib.io.Util.reverseBytesLong;
...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
    ...
    @Override
    public long readToDelimiter(Sink sink, byte b, long fromIndex) throws IOException {
        long byteCount = indexOf(b, fromIndex);
        if (byteCount > 0) {
            sink.write(this, byteCount);
        }
        return byteCount;
    ...
    public String readUtf8LineStrict() throws EOFException {
        ...
        if (newline == -1L) {
            ...
            throw new EOFException("newline not found: size=" + size()
    ...
    String readUtf8Line(long newline) throws EOFException {
        if (newline > 0L && getByte(newline - 1) == CR) {
    ...
    @Override
    public byte[] readToDelimiter(byte delimiter) throws EOFException {
        long l = indexOf(delimiter);
        if (l == -1L) {
            return size != 0L ? readByteArray(size) : null;
        }
        byte[] b = readByteArray(l);
        skip(1);
        return b;
    }
