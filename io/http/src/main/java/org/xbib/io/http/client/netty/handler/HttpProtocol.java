...
import org.xbib.io.http.client.ntlm.NtlmEngine;
import org.xbib.io.http.client.proxy.ProxyServer;
import org.xbib.io.http.client.spnego.SpnegoEngine;
import org.xbib.io.http.client.spnego.SpnegoEngineException;
...
import static org.xbib.io.http.client.util.AuthenticatorUtils.getHeaderWithPrefix;
import static org.xbib.io.http.client.util.HttpConstants.Methods.CONNECT;
import static org.xbib.io.http.client.util.HttpConstants.ResponseStatusCodes.CONTINUE_100;
import static org.xbib.io.http.client.util.HttpConstants.ResponseStatusCodes.OK_200;
import static org.xbib.io.http.client.util.HttpConstants.ResponseStatusCodes.PROXY_AUTHENTICATION_REQUIRED_407;
import static org.xbib.io.http.client.util.HttpConstants.ResponseStatusCodes.UNAUTHORIZED_401;
...
public final class HttpProtocol extends Protocol {
    ...
    private boolean updateBodyAndInterrupt(NettyResponseFuture<?> future, AsyncHandler<?> handler, HttpResponseBodyPart bodyPart) throws Exception {
        boolean interrupt = handler.onBodyPartReceived(bodyPart) != AsyncHandler.State.CONTINUE;
        if (interrupt) {
    ...
    private boolean exitAfterHandlingStatus(Channel channel, NettyResponseFuture<?> future, HttpResponse response, AsyncHandler<?> handler, NettyResponseStatus status,
        ...
        return !future.getAndSetStatusReceived(true) && handler.onStatusReceived(status) != AsyncHandler.State.CONTINUE;
    }
    ...
    private boolean exitAfterHandlingReactiveStreams(Channel channel, NettyResponseFuture<?> future, HttpResponse response, AsyncHandler<?> handler, HttpRequest httpRequest)
        ...
        if (handler instanceof StreamedAsyncHandler) {
            ...
            return streamedAsyncHandler.onStream(publisher) != AsyncHandler.State.CONTINUE;
        }
        return false;
    }
