...
public class SOCKS5Connector extends FTPConnector {
     ...
     */
    public SOCKS5Connector(Logger logger, String socks5host, int socks5port, String socks5user, String socks5pass) {
        super(logger);
    ...
    private Socket socksConnect(String host, int port, boolean forDataTransfer) throws IOException {
        ...
        try {
            if (forDataTransfer) {
                socket = createDataSocket(socks5host, socks5port);
            } else {
                socket = createCommSocket(socks5host, socks5port);
            }
