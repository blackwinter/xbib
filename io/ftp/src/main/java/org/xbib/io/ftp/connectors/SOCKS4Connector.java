...
public class SOCKS4Connector extends FTPConnector {
     ...
     */
    public SOCKS4Connector(Logger logger, String socks4host, int socks4port, String socks4user) {
        super(logger);
    ...
    private Socket socksConnect(String host, int port, boolean forDataTransfer) throws IOException {
        ...
        try {
            if (forDataTransfer) {
                socket = createDataSocket(socks4host, socks4port);
            } else {
                socket = createCommSocket(socks4host, socks4port);
            }
