...
import org.xbib.util.persistent.AbstractRedBlackTree.Color;
...
public class PersistentTreeMapTest extends AbstractPersistentMapTestHelper<PersistentTreeMap<Integer, Integer>> {

    private static Integer blacksOnPath = null;

    private static Iterable<Integer> keys(Iterable<Map.Entry<Integer, Integer>> entries) {
        return FluentIterable.transform(entries, MapUtils.<Integer>mapKeyFunction());
    }

    private static synchronized void assertNodeProperties(Node<Integer, Integer> node) {
        blacksOnPath = null;
        if (node != null) {
    ...
    private static void assertNodeProperties(Node<Integer, Integer> node, int blacks) {
        ...
        if (leaf) {
            if (blacksOnPath == null) {
                ...
                assertThat(blacks, equalTo(blacksOnPath));
            }
        }
    }

    private static void assertBlack(Node<?, ?> node) {
